/*
  Warnings:

  - The primary key for the `Configuration` table will be changed. If it partially fails, the table could be left without primary key constraint.

*/
BEGIN TRY

BEGIN TRAN;

-- RedefineTables
BEGIN TRANSACTION;
DECLARE @SQL NVARCHAR(MAX) = N''
SELECT @SQL += N'ALTER TABLE '
    + QUOTENAME(OBJECT_SCHEMA_NAME(PARENT_OBJECT_ID))
    + '.'
    + QUOTENAME(OBJECT_NAME(PARENT_OBJECT_ID))
    + ' DROP CONSTRAINT '
    + OBJECT_NAME(OBJECT_ID) + ';'
FROM SYS.OBJECTS
WHERE TYPE_DESC LIKE '%CONSTRAINT'
    AND OBJECT_NAME(PARENT_OBJECT_ID) = 'Configuration'
    AND SCHEMA_NAME(SCHEMA_ID) = 'dbo'
EXEC sp_executesql @SQL
;
CREATE TABLE [dbo].[_prisma_new_Configuration] (
    [id] NVARCHAR(1000) NOT NULL,
    [scaleDown] FLOAT(53) NOT NULL,
    [logoPosition] NVARCHAR(1000) NOT NULL,
    [logoImage] NVARCHAR(1000) NOT NULL,
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [Configuration_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [Configuration_pkey] PRIMARY KEY CLUSTERED ([id])
);
IF EXISTS(SELECT * FROM [dbo].[Configuration])
    EXEC('INSERT INTO [dbo].[_prisma_new_Configuration] ([createdAt],[id],[logoImage],[logoPosition],[scaleDown],[updatedAt]) SELECT [createdAt],[id],[logoImage],[logoPosition],[scaleDown],[updatedAt] FROM [dbo].[Configuration] WITH (holdlock tablockx)');
DROP TABLE [dbo].[Configuration];
EXEC SP_RENAME N'dbo._prisma_new_Configuration', N'Configuration';
COMMIT;

COMMIT TRAN;

END TRY
BEGIN CATCH

IF @@TRANCOUNT > 0
BEGIN
    ROLLBACK TRAN;
END;
THROW

END CATCH
